    ...
    <div className='w-full flex justify-center text-center'>
      <div className='m-[180px]'>
                ...
                <div className='border-2 border-black-200 py-2 pl-3 pr-5 rounded-md flex bor'>
                  ...
                  <div>
                    <div className='flex justify-start'>
                      <span className='font-bold text-left'>{user?.displayName}</span>
                    </div>
                    <div className='flex justify-start'>
                      <span className='text-black-700 text-left'>{user?.email}</span>
                    </div>
                  </div>
                </div>
              </>
        ...
        <div className='pb-2'>
          {
            Object.keys(user || {}).length === 0 ?
            isLoggedIn? '' : (<Button onClick={onClickGoogleSignIn}>Google Sign In</Button>) :
            (<Button variant='muted' type='translucent' onClick={logout}>Logout</Button>)
          }
        </div>
        <Button onClick={() => router.push('/linky')}>Go back to Linky Page</Button>
      </div>
    </div>
  );
}
